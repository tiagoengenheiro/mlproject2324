...
#RS = StandardScaler(with_centering=True, with_scaling=False)
RS = StandardScaler(with_mean=True, with_std=True) ##RS = StandardScaler(with_centering=True, with_scaling=False)
#Outliers
...
#results=np.hstack((model1.predict(X_train_init),model2.predict(X_train_init)))
results = np.abs(results-y_train_init)
